...
// MyArray constructor to create a MyArray of size elements defaulted to 0
MyArray::MyArray(size_t size)
    : m_size{size}, m_ptr{make_unique<int[]>(size)}{
    cout << "MyArray(size_t) constructor\n";
}

// MyArray constructor that accepts an intializer list
MyArray::MyArray(initializer_list<int> list)
    ...
    // copy list argument's elements into m_ptr's underlaying int array
    // m_ptr.get() returns the int array's starting memory location
    copy(begin(list), end(list), m_ptr.get());
}

// copy constructor: must recieve a reference to a MyArray
MyArray::MyArray(const MyArray& original)
    ...
    // copy orginal's elements into m_ptr's underlying int array
...
// copy assignment operator= implemented with copy-and-swap idiom
 MyArray&  MyArray::MyArray::operator=(const MyArray& right) {
...
MyArray& MyArray::operator=(MyArray&& right) noexcept {
     ...
     if (this != &right) { // avoid self-assignment
        ...
        m_size = std::exchange(right.m_size, 0); // indicates right is empty
...
//postincrement every element, and rturn copy of original MyArray
// Note the hack as the int parameter is never used
...
void swap(MyArray& a, MyArray& b) noexcept {
    ...
    a.m_ptr.swap(b.m_ptr); // swapusing unite_ptr swap member function
}